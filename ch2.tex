...
\begin{figure}
  \[
  \begin{array}{lrrll}
    \text{Variable}   &  V & ::= & x~|~y~|~z~|~\cdots & \\[0.5mm]
    \text{Expression} &  E & ::= & V                  & \smallparen{variable} \\
                      &    &   | & \lambda V : T.\ E  & \smallparen{abstraction} \\
                      &    &   | & E\ E               & \smallparen{application}
    \\ \\
    \text{Type}       &  T & ::= & \alpha             & \smallparen{type variable} \\
                      &    &   | & T \to T            & \smallparen{function type}
  \end{array}
  \]
\caption{An example EBNF-like syntax description of the simply-typed lambda calculus (STLC).}
\label{fig:syntax}
\end{figure}
...
\begin{figure}
  ...
  \end{mathpar}
\caption{An example type rule denoting STLC-style type checking, rendered with the \texttt{mathpartir} package.}
\label{fig:typerule}
\end{figure}
